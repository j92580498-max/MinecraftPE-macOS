...
// Android / Raspberry Pi / iOS use OpenGL ES.
// macOS (Apple but not TARGET_OS_IPHONE) uses desktop OpenGL.
...
// Other systems might run it, if they #define OPENGL_ES
    ...
    // macOS desktop (OS X 10.9 Mavericks and later): legacy / compatibility
    // OpenGL 2.1 from OpenGL.framework. We keep using the GLES 1.1 style
    // fixed-function pipeline; the engine never touches shaders.
	#define USE_VBO
	#define GL_QUADS 0x0007
	#include <OpenGL/gl.h>
